...
TEST(AzBlobUrl, to_string)
{
  ...
  const rosbaz::DataSpan span(reinterpret_cast<const uint8_t*>(expected.c_str()), expected.size());
...
TEST(AzBlobUrl, read_little_endian_uint32_t)
{
  ...
  std::copy(&expected, &expected + 1, reinterpret_cast<uint32_t*>(data.begin()));
