...
class AzReader : public IReader {
 public:
  ...
  explicit AzReader(const AzBlobUrl &blob_url, const std::string &account_key,
                    const std::string &token,
                    std::unique_ptr<ICacheStrategy> cache_strategy);

  size_t size() override;

  std::int32_t num_requests() const { return num_requests_; }
  std::int64_t num_bytes() const { return num_bytes_; }
