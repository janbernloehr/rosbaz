...
namespace bag_parsing {
  ...
  /// Implements a subset of the functionality provided by rosbag::Bag offloading the actual data
  /// reading to an implementation of rosbaz::io::IReader provided by the user.
class AzBag {
 public:
  friend class View;
...
/// Create a bag instance using the given reader.
  static AzBag read(rosbaz::io::IReader& reader, bool read_chunk_indices = true);
 ...
 private:
  ...
  /// Parses the indexes following a chunk record and populates connection_indexes_; also sets
  /// message_records of the given ChunkExt.
  void parseIndexSection(ChunkExt& chunk_ext,
