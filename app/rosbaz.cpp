...
void print_bag(const rosbaz::bag_parsing::AzBag& bag) {
  ...
  std::cout << "size:     " << static_cast<float>(bag.getFileSize()) / 1024.f / 1024.f / 1024.f
            << " GB\n";
...
void info_command(const CommonOptions& common_options, const InfoOptions& info_options) {
  ...
  rosbaz::io::AzReader az_reader{url, common_options.account_key, common_options.token};
  auto az_bag = rosbaz::bag_parsing::AzBag::read(az_reader, false);
  print_bag(az_bag);

  ROS_INFO_STREAM("Requests: " << az_reader.num_requests() << " bytes " << az_reader.num_bytes());
}
...
void play_command(const CommonOptions& common_options, const PlayOptions& play_options) {
  ...
  // we first create a full blown view to obtain the time range of the messages and then filter
  // that view
  rosbaz::bag_parsing::View full_view{az_bag, az_reader};

  const auto initial_time =
      full_view.getBeginTime() + ros::Duration(play_options.start_offset.value_or(0));
  ros::Time finish_time = ros::TIME_MAX;
  if (play_options.duration) {
    finish_time = initial_time + ros::Duration(*play_options.duration);
  }

  rosbaz::bag_parsing::View filtered_view{az_bag, az_reader, initial_time, finish_time};

  std::unordered_map<std::string, ros::Publisher> publishers;
  for (const auto* connection_info : filtered_view.getConnections()) {
    ros::AdvertiseOptions options(connection_info->topic, 50, connection_info->md5sum,
...
int main(int argc, char** argv) {
  ...
  app.add_option(
      "--token", common_options.token,
      "The storage account bearer token, which you can obtain by running\naz account "
      "get-access-token --resource https://storage.azure.com/ -o tsv --query accessToken");


  InfoOptions info_options;

  CLI::App* info =
      app.add_subcommand("info", "Summarize the contents of one bag file.")->fallthrough();

  info->add_option("blob_url", info_options.blob_url, "The blob url (may including SAS token)")
      ->required();
  ...
  play->add_option("blob_url", play_options.blob_url, "The blob url (may including SAS token)")
      ->required();
