...
///
/// \detail
/// The size of the progress bar is inferred automatically from the maximum number of steps, the
/// current step and the width of the terminal.
///
/// The progress bar can be incremented using the ++ and += operators as well as setCurrentStep.
class ProgressBar {
 public:
  ...
  void setCurrentStep(int value) { current_step_ = value; }
  int getMaxSteps() const { return max_steps_; }

  int getStartStep() const { return start_step_; }
  int getStopStep() const { return stop_step_; }
