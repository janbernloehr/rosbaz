...
std::vector<rosbaz::io::byte> MessageInstance::read_subset(
  ...
  const auto header = rosbaz::bag_parsing::Header::parse(
      m_header_buffer_and_size->header_buffer);
  ...
  return m_reader->read(
      record_offset + m_header_buffer_and_size->data_offset() + offset, size);
}
