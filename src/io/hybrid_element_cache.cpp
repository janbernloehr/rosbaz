...
bool HybridElementCacheStrategy::retrieve(rosbaz::io::byte* buffer, size_t offset, size_t count)
{
  if (count > max_small_element_size_)
  {
    ...
    std::copy_n(large_cache_found->data.begin() + (offset - large_cache_found->offset), count,
                buffer);
  ...
  else
  {
    ...
    std::copy_n(small_cache_found->data.begin() + (offset - small_cache_found->offset), count,
                buffer);
