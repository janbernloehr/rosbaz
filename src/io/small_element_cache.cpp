...
SmallElementCacheStrategy::SmallElementCacheStrategy(
    const size_t max_element_size, const size_t max_elements)
    : max_element_size_(max_element_size), max_elements_(max_elements) {}

bool SmallElementCacheStrategy::retrieve(rosbaz::io::byte *buffer,
  ...
  auto cache_found = std::find_if(
      cache_.begin(), cache_.end(), [offset, count](const CacheEntry &entry) {
...
void SmallElementCacheStrategy::update(std::vector<rosbaz::io::byte> data,
                                       size_t offset) {
