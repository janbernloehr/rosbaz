...
HeaderBufferAndSize IReader::read_header_buffer_and_size(size_t offset) {
  ...
  result.header_buffer =
      read(offset + sizeof(uint32_t), result.header_size + sizeof(uint32_t));

  result.data_size = rosbaz::io::read_little_endian<uint32_t>(
      result.header_buffer, result.header_size);
