...
Record Record::parse(rosbaz::DataSpan source) {
  ...
  assert(source.size() >= sizeof(uint32_t) + record.header_length + sizeof(uint32_t));
  record.data_length = rosbaz::io::read_little_endian<std::int32_t>(
      source, sizeof(std::uint32_t) + record.header_length);

  ROS_DEBUG_STREAM("record: header length: " << record.header_length << " data length: "
                                           << record.data_length << " source: " << source.size());

  record.data = 
      source.subspan(
      +sizeof(std::uint32_t) + static_cast<size_t>(record.header_length) + sizeof(std::uint32_t),
