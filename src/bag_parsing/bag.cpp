...
void AzBag::parseFileHeaderRecord(const Record& file_header_record) {
  ...
  chunk_count_ =
      rosbaz::io::read_little_endian<uint32_t>(h.fields.at(rosbag::CHUNK_COUNT_FIELD_NAME));
  connection_count_ =
      rosbaz::io::read_little_endian<uint32_t>(h.fields[rosbag::CONNECTION_COUNT_FIELD_NAME]);
  index_data_pos_ =
      rosbaz::io::read_little_endian<uint64_t>(h.fields[rosbag::INDEX_POS_FIELD_NAME]);
}
...
void AzBag::parseIndexSection(ChunkExt& chunk_ext,
  ...
  while (offset < chunk_index.size()) {
    ...
    if (index_header.op != rosbag::OP_INDEX_DATA) {
      ...
      msg << "Unexpected op code " << static_cast<int>(index_header.op) << " in index section";
    ...
    const uint32_t count =
        rosbaz::io::read_little_endian<uint32_t>(index_header.fields.at(rosbag::COUNT_FIELD_NAME));
  ...
  for (size_t i = 0; i < offsets.size() - 1; ++i) {
    record_sizes.emplace_back(
        MessageRecordInfo{offsets[i], static_cast<uint32_t>(offsets[i + 1] - offsets[i])});
  }

  record_sizes.emplace_back(MessageRecordInfo{
      offsets.back(),
      static_cast<uint32_t>(index_offset - offsets.back() - chunk_ext.chunk_info.pos)});
}

void AzBag::parseChunkIndices(rosbaz::io::IReader& reader) {
  // keep the position of the next chunk to determine the size of the index record section.
  ...
  for (const auto& chunk_info : chunk_infos_) {
    ...
    const auto chunk_header =
        as_chunk_header(chunk_header_raw, chunk_header_buffer_and_size.data_size);
    ...
    const uint64_t data_offset = chunk_info.pos + sizeof(uint32_t) +
                                 chunk_header_buffer_and_size.header_size + sizeof(uint32_t);
