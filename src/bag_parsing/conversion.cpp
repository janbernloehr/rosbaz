...
rosbag::ChunkHeader as_chunk_header(const Header &header,
  ...
  const auto &compression_field =
      header.fields.at(rosbag::COMPRESSION_FIELD_NAME);
  chunk_header.compression = rosbaz::io::to_string(compression_field);

  chunk_header.compressed_size = compressed_size;
  chunk_header.uncompressed_size = rosbaz::io::read_little_endian<uint32_t>(
      header.fields.at(rosbag::SIZE_FIELD_NAME));

  return chunk_header;
}

rosbag::ConnectionInfo as_connection_info(const Header &header,
...
rosbag::ChunkInfo as_chunk_info(const Header &header, rosbaz::DataSpan data) {
  ...
  auto ver = rosbaz::io::read_little_endian<uint32_t>(
  ...
  info.pos = rosbaz::io::read_little_endian<uint64_t>(
      header.fields.at(rosbag::CHUNK_POS_FIELD_NAME));

  info.start_time = unpack_time(rosbaz::io::read_little_endian<uint64_t>(
      header.fields.at(rosbag::START_TIME_FIELD_NAME)));
  info.end_time = unpack_time(rosbaz::io::read_little_endian<uint64_t>(
      header.fields.at(rosbag::END_TIME_FIELD_NAME)));

  ROS_DEBUG_STREAM(" pos: " << info.pos
                            << " start: " << info.start_time.toNSec()
                            << " end: " << info.end_time.toNSec());

  auto count = rosbaz::io::read_little_endian<uint32_t>(
      header.fields.at(rosbag::COUNT_FIELD_NAME));

  for (int32_t i = 0; i < count; ++i) {
    const auto conn_id =
        rosbaz::io::read_little_endian<uint32_t>(data.subspan(8 * i, 4));
